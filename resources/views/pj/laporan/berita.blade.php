...
<div class="card border-0 shadow-sm mb-4" style="border-radius: 12px;">
    <div class="card-body p-3">
        <form action="{{ route('pj.laporan.berita') }}" method="GET" class="row g-3 align-items-end">
            <div class="col-md-5">
...
<div class="card border-0 shadow-sm" style="border-radius: 15px; overflow: hidden;"><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0" style="font-size: 0.9rem;"><thead class="text-white" style="background-color: #43a047;"><tr><th class="ps-4 py-3" style="width: 60px;">No</th><th>Judul Artikel</th><th>Tanggal Rilis</th><th class="text-center">Total Klik</th></tr></thead><tbody>
@forelse($beritas as $i => $b)<tr><td class="ps-4 text-muted">{{ $beritas->firstItem() + $i }}</td><td class="fw-bold text-dark">{{ $b->judul }}</td><td>{{ $b->created_at->format('d-m-Y') }}</td><td class="text-center"><span class="badge bg-light text-dark border px-3 py-2 fw-bold">👁️ {{ $b->views ?? 0 }}</span></td></tr>@empty<tr><td colspan="4" class="text-center py-4 text-muted">Belum ada data berita.</td></tr>@endforelse
